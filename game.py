...
def opening_line():
    ...
    """
    print("""\nYou are in search of Gul'dan the Warlock, the leader of the Horde, who destroyed your clan. 
...
'Class_Level': 'Prince'}
    ...
    rogue = ['Shadow', 'Assassin', 'Nightslayer']
    if character['Class'] == 'Paladin':
...
XP required to level up is {character['Level-Up-XP'] - character['Current-XP']}.""")
        if character['Level'] == 3:
            print("\nYou sense Gul'Dan at the edge of the Barrens, on top of the Blackrock Mountain. "
...
def describe_current_location(board: dict, character: dict):
    ...
    >>> gamer = {'Name': 'Aditya', 'X-coordinate': 0, 'Y-coordinate': 0, 'HP': 2}
    >>> describe_current_location(game_board, gamer)
    Aditya is on the way to find Gul'dan, the Warlock at (0, 0).

    >>> game_board_2 = make_board(10, 10)
    >>> character_info = {'Name': 'Aditya', 'X-coordinate': 9, 'Y-coordinate': 9, 'HP': 2}
    >>> describe_current_location(game_board_2, character_info)
    Aditya is at the foot of the Blackrock Mountain, the lair of Gul'dan at (9, 9).
    """
...
def get_user_choice() -> str:
    ...
    while True:
        ...
        if user_choice == "1":
            ...
            break
        elif user_choice.lower() == "score" or user_choice.lower() == "quit":
            break
        else:
...
def move_character(character: dict, direction: str):
    """
    Update the character coordinates whenever a valid move is made.
    ...
    :param character: A dictionary
    :param direction: A positive int
    :precondition: The character parameter must be a dictionary with at least keys of X and Y coordinate. The direction
    parameter must be a positive integer between 1 and 4. It representx the 4 directions.
    :postcondition: Change the coordinates according to the move made by the user.

    >>> gamer = {'X-coordinate': 0, 'Y-coordinate': 0}
    >>> direction_move = "2"
...
def increase_health_and_xp(character: dict):
    """
    Increase the character health and current XP.

    :param character: A dictionary
    :precondition: The character parameter must be a dictionary with keys of X and Y coordinate, level, current XP and
    the XP required to level up, the class, health, max health and damage for the character.
    :postcondition: Increase the character health by 5 and character current XP by 10.

    >>> character_info = {'Health': 0, 'Current-XP': 0}
    >>> increase_health_and_xp(character_info)
    >>> character_info
    {'Health': 5, 'Current-XP': 10}
    """
    character['Health'] += 5
...
def fight_or_flee() -> bool:
    ...
    return True if user_choice == '5' else False
...
def guessing_game_select(character: dict):
   ...
   :precondition: The character parameter must be a dictionary with keys of X and Y coordinate, level, current XP and
    ...
    guess = input(f"Enter an option between 1 and 3 to guess the correct side:\n ")
...
def make_game_enemy(character: dict) -> dict:
    ...
    >>> character_info = {'X-coordinate': 0, 'Y-coordinate': 0, 'Level': 1, 'Current-XP': 0, 'Level-Up-XP': 1000, \
    'Class': 'Paladin', 'Health': 1000, 'Max_Health': 1000, 'Damage': 95, 'Passive_Power': 'Blessing of the Light'}
    >>> make_game_enemy(character_info)
    {'Enemy_Health': 500, 'Enemy_Damage': 48}
    """
    return {'Enemy_Health': round(character['Max_Health'] / 2), 'Enemy_Damage': round(character['Damage'] / 2)}
...
def fighting_game_damage(character: dict, enemy: dict):
    ...
    health_cap(character)
    print(f"Your health is {round(character['Health'])} and your enemy has {round(enemy['Enemy_Health'])} health "
          f"remaining.")
...
def enemy_flee_damage(character: dict, enemy: dict):
    ...
    if flee == 1:
        ...
        print("However, you were damaged while fleeing the battle.")
    else:
        print("You managed to get away without taking any damage.")
...
def fighting_game(character: dict, enemy: dict):
    ...
    A function to simulate the scenario of confronting an enemy and fighting it.
...
def make_game_boss(character: dict) -> dict:
    ...
    >>> character_info = {'X-coordinate': 0, 'Y-coordinate': 0, 'Level': 1, 'Current-XP': 0, 'Level-Up-XP': 1000, \
    'Class': 'Paladin', 'Health': 1200, 'Max_Health': 1000, 'Damage': 95, 'Passive_Power': 'Blessing of the Light'}
    >>> make_game_boss(character_info)
    <BLANKLINE>
    You face the leader of the Horde, Gul'dan
    {'Enemy_Health': 2500, 'Enemy_Damage': 119}
    """
    print("\nYou face the leader of the Horde, Gul'dan.")
...
def boss_dead(character: dict, boss: dict) -> bool:
    ...
    >>> character_info = {'X-coordinate': 0, 'Y-coordinate': 0, 'Level': 1, 'Current-XP': 0, 'Level-Up-XP': 1000, \
    'Class': 'Paladin', 'Health': 1200, 'Max_Health': 1000, 'Damage': 95, 'Passive_Power': 'Blessing of the Light'}
    >>> boss_info =  {'Enemy_Health': 0, 'Enemy_Damage': 119}
    >>> boss_dead(character_info, boss_info)
    <BLANKLINE>
    ...
    >>> character_info = {'X-coordinate': 0, 'Y-coordinate': 0, 'Level': 1, 'Current-XP': 0, 'Level-Up-XP': 1000, \
    'Class': 'Paladin', 'Health': 1200, 'Max_Health': 1000, 'Damage': 95, 'Passive_Power': 'Blessing of the Light'}
    >>> boss_info =  {'Enemy_Health': 10, 'Enemy_Damage': 119}
    >>> boss_dead(character_info, boss_info)
    False
    """
...
def check_if_reached_boss(rows: int, columns: int, character: dict) -> bool:
    ...
    >>> board_rows = 3
    >>> board_columns = 3
    >>> gamer = {'X-coordinate': 0, 'Y-coordinate': 0, 'Level': 1, 'Current-XP': 0, 'Level-Up-XP': 1000, \
    'Class': 'Paladin', 'Health': 1200, 'Max_Health': 1000, 'Damage': 95, 'Passive_Power': 'Blessing of the Light'}
    ...
    >>> board_rows = 3
    >>> board_columns = 3
    >>> gamer = {'X-coordinate': 2, 'Y-coordinate': 2, 'Level': 1, 'Current-XP': 0, 'Level-Up-XP': 1000, \
    'Class': 'Paladin', 'Health': 1200, 'Max_Health': 1000, 'Damage': 95, 'Passive_Power': 'Blessing of the Light'}
    >>> check_if_reached_boss(board_rows, board_columns, gamer)
    True
    """
...
def guess_coordinates() -> list:
    ...
    >>> guess_coordinates()
    [(4, 4), (4, 9), (4, 15), (4, 20), (9, 4), (9, 9), (9, 15), (9, 20), (15, 4), (15, 9), (15, 15), (15, 20), \
(20, 4), (20, 9), (20, 15), (20, 20)]
    """
    guess_number = [4, 9, 15, 20]
    guess_coordinates_list = itertools.product(guess_number, guess_number)
    return list(guess_coordinates_list)


def guessing_game_runner(character: dict) -> bool:
    """
...
def game():
    ...
    while is_alive(character) and not achieved_goal:
        ...
        if valid_move:
            ...
            describe_current_location(board, character)
            if guessing_game_runner(character):
                guessing_game(character)
            increase_health_and_xp(character)
            health_cap(character)
